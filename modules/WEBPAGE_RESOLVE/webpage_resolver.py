...
class WebpageResolver(DataSource):
    ...
    def get_html(webpage):
        ...
        all_pages = glob.glob("modules/WEBPAGE_RESOLVE/cache/*")
        if any(map(lambda x: page_name == os.path.split(x)[-1], all_pages)):
            with open("modules/WEBPAGE_RESOLVE/cache/"+page_name, "r") as f:
                return f.read()

        page = requests.get(webpage)
        with open("modules/WEBPAGE_RESOLVE/cache/"+page_name, "w") as f:
            f.write(page.text)
